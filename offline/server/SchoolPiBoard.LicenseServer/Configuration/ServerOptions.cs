...
public sealed class ServerOptions
{
    ...
    /// товар в чеке.
    ///
    /// Запасного варианта нет: не настроен — подписки не продаются.
    /// Провести подписку через кассу лицензий значило бы выбить чек не за
    ...
    public static ServerOptions Load(IConfiguration configuration, bool development)
    {
        var options = new ServerOptions
        {
            ...
            Smtp = new SmtpOptions
            {
                ...
                FromName = configuration["Smtp:FromName"] ?? "Доска Пи",
                Subject = configuration["Smtp:Subject"] ?? "Ваш ключ «Доска Пи»",
            ...
            Robokassa = new RobokassaOptions
            {
                ...
                Description = configuration["Robokassa:Description"] ?? "Лицензия «Доска Пи» (бессрочная)",
