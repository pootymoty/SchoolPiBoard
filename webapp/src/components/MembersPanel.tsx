...
interface Props {
  boardId: string;
  /** Управлять составом может только владелец; остальным панель доступна только на чтение. */
  canManage: boolean;
...
export function MembersPanel({ boardId, canManage, currentUserId }: Props): ReactElement {
  ...
  const invite = async (event: FormEvent) => {
    ...
    try {
      await api<Member>(`/boards/${boardId}/members`, {
        method: 'POST',
        body: { email, role },
      });
      setEmail('');
  ...
  const changeRole = async (member: Member, next: BoardRole) => {
    setError(null);
    try {
      await api<Member>(`/boards/${boardId}/members/${member.userId}`, {
        method: 'PATCH',
        body: { role: next },
      });
      await load();
          ...
          <li key={member.userId} className="member-row">
            ...
            {canManage && member.role !== 'owner' ? (
              <div className="row">
                <select
                  value={member.role}
                  onChange={(event) => void changeRole(member, event.target.value as BoardRole)}
                >
      ...
      {canManage ? (
        <form className="invite-form" onSubmit={invite}>
          ...
          <p className="muted small">
            Пригласить можно того, кто уже зарегистрирован: ссылки-приглашения появятся позже.
          </p>
        </form>
      ) : null}
